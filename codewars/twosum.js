...
var maxSlidingWindow = function(nums, k) {
  ...
  for (let i = 0; i < nums.length; i++) {
      ...
      if (i >= k - 1) {    
          const max = queue[0];      
          result.push(max);
          console.log(">>>>>>>>>>>>>>>>> ","nums: ", nums,nums[i - k + 1], "i-k+1: ", i-k+1, "|||||" ,"max: ",max,"queue: ", queue, "result: ",result)
          if (nums[i - k + 1] === max) {
              queue.shift();
          }
      }
...
function twoSumWithHash(nums,target){
  ...
  let map = new Map;
    for (var i = 0; i < nums.length; i++) {
        ...
        console.log("2. were gonna set each nums[i] element as a key in a hashmap and the value is its index: ", "(",nums[i],",",i,")")
      
        if (map.has(complement)) {
          map.set(nums[i], i);
          console.log("map: ", map)
          console.log(`3. We will then keep checking: if map.has(compliment) == true: this means we found, compliment: ${complement}, as one of the keys in the hashmap`, map,`if false, we repeat. (current output: ${map.has(complement)}),`)
          console.log(`Here we found the compliment: ${complement}, as one of the keys in the hashmap above! We then can conclude that`)
          console.log(`map.get(complement) is like map.get(${complement}) and it returns the value of the complement: ${map.get(complement)}`)
          console.log(`this is the index in nums array, that can be added to nums[current iteration i], ${i} that add up to target: ${target}`)
          console.log(`simply put: nums[map.get(complement)] + nums[i] = target.  REAL VAL computation:  ${nums[map.get(complement)]} + ${nums[i]} = ${nums[map.get(complement)] + nums[i]}`)
